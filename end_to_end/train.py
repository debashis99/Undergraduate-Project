...
with tf.Session() as sess:
    ...
    for (x, y) in train_set:
        im = imageio.imread(_dir_data  + 'images_processed/' + str(x)) # a numpy array
        im = greyscale(im)
        s = str(im.shape)
        print(s)
        if s not in buckets:
    ...
    for epoch in range(20):
        ...
        for b_id, bucket in buckets.items():
            print("Bucket shape:", b_id, " size: ", len(bucket))
            i_img = []
            formul = []
            for i, (x,y) in enumerate(bucket):
                ...
                i_img = i_img + [x]                
                formul = formul + [y]
                if( (len(i_img) == 12) or ( i == len(bucket) -1) ):
                    sys.stdout.write(' .. ')
                    formul, formul_len = pad_batch_formulas(formul, vocab["_PAD"], vocab["_END"])
                    fd = { img :i_img, formula : formul, formula_length :formul_len }
                    err = sess.run([train_op,loss], feed_dict=fd)

                    sys.stdout.write(str(err[1]))

                    # print(sess.run(test_outputs,feed_dict=fd ))
                    i_img = []
                    formul = []
